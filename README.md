...
| 文档 | 说明 |
|---|---|
| 🌐 [**网站（本地）**](site/index.html) | 图文版计划站：卡通插图、路线图、可勾选装备清单 —— 双击 `site/index.html` 打开（**前后端分离改造中，见下方说明**） |
| 🏗️ [**任务协调**](tasks/README.md) | 前后端分离改造的任务清单（跨 session / 并行协作入口） |
...
│   ├── 装备总览.md          # 装备台账
│   └── 采购清单.md          # 采购计划
├── web/                    # 新前端（React + Vite + TS，前后端分离改造中）
├── backend/                # 新后端（FastAPI，提供内容 API，改造中）
├── tasks/                  # 前后端分离改造任务协调（跨 session / 并行）
├── site/                   # 旧图文版网站（纯静态，过渡期保留）
...
- [ ] **前后端分离改造**（React + FastAPI，任务见 [tasks/](tasks/README.md)）
- [ ] 确认现有露营装备 → 出最终采购单
