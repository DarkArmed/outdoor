/* ============================================================
   scenes.js — 卡通插图生成器
   每个主题返回一张 SVG 场景（viewBox 800×420），全部内联矢量，
   离线可用。风格：扁平、圆角、高饱和、微笑太阳。
   ============================================================ */
...
function scMoon(x, y) {
  ...
  ${Array.from({ length: 14 }, (_, i) => {
    const sx = 40 + (i * 53) % 730, sy = 25 + (i * 37) % 130;
    return `<circle cx="${sx}" cy="${sy}" r="${1.5 + (i % 3)}" fill="#FFF9DB"/>`;
  }).join('')}`;
}

function scCloud(x, y, s = 1) {
  return `
...
function scTree(x, y, s = 1, color = SC.leaf) {
  return `
...
function scPine(x, y, s = 1, color = SC.grassDk) {
  return `
  <g transform="translate(${x},${y}) scale(${s})">
...
function scPerson(x, y, s, shirt, opts = {}) {
...
function scDadKid(x, y, s = 1, mom = false) {
  const dadX = mom ? x - 40 : x - 26;
  const kidX = mom ? x + 4 : x + 24;
  const kidS = s * 0.72;
  let out = scPerson(dadX, y, s, SC.dadShirt)
  ...
  if (mom) {
    out += scPerson(x + 40, y, s * 0.95, SC.momShirt, { hair: '#6B4F2E' })
...
function scFish(x, y, s = 1, color = '#FF8FAB') {
  return `
  <g transform="translate(${x},${y}) scale(${s})">
...
function scCar(x, y, s = 1, color = '#FF6B6B') {
  return `
  <g transform="translate(${x},${y}) scale(${s})">
...
function scBike(x, y, s = 1, color = '#5B8DEF') {
  return `
...
function scKite(x, y, s = 1, color = '#FF8FAB') {
  return `
  <g transform="translate(${x},${y}) scale(${s})">
...
const SCENES = {
  /* 溯溪玩水 */
  water(mom) {
    return scSky() + scSun(110, 90) + scCloud(320, 70) + scCloud(600, 110, 0.8)
      + scHills() + scRiver(320, 100)
      + scTree(90, 300, 1.1) + scPine(720, 300, 1.2) + scTree(660, 310, 0.8)
      + scFish(300, 370) + scFish(480, 385, 0.8, '#FFD93D')
      + scDadKid(560, 312, 1, mom);
  },

  /* 山野徒步 */
  hike(mom) {
    return scSky() + scSun(660, 80) + scCloud(200, 80) + scCloud(460, 50, 0.7)
      + scHills()
      + `<path d="M80 420 Q 240 330 380 350 T 720 300" stroke="#E9D8A6" stroke-width="26" fill="none" stroke-linecap="round"/>`
      + scPine(120, 340, 1.1) + scTree(300, 330, 0.9) + scPine(560, 320, 0.9) + scTree(700, 300, 1)
      + scDadKid(430, 342, 1, mom);
  },

  /* 露营（白天） */
  camp(mom) {
    return scSky() + scSun(120, 90) + scCloud(420, 60) + scCloud(640, 110, 0.75)
      + scHills() + scPine(90, 330, 1.2) + scTree(730, 320, 1.1)
      + scTent(300, 360, 1.2) + scCampfire(520, 380, 0.9)
      + scDadKid(180, 345, 1, mom);
  },

  /* 露营（夜晚星空） */
  campnight(mom) {
    return scSky(true) + scMoon(120, 80) + scHills(true)
      + scPine(90, 330, 1.2, '#1E2B4A') + scTree(730, 320, 1.1, '#1E2B4A')
      + scTent(300, 360, 1.2) + scCampfire(520, 380, 1)
  ...
  /* 骑行 */
  cycle(mom) {
    return scSky() + scSun(660, 85) + scCloud(220, 70) + scCloud(500, 110, 0.8)
      + scHills()
      ...
      + scTree(110, 330, 1) + scTree(700, 330, 0.9)
      + scBike(340, 384, 1.1) + scBike(500, 390, 0.8, '#FF6B6B')
  ...
  /* 草原 */
  grassland(mom) {
    return scSky() + scSun(120, 80) + scCloud(360, 60) + scCloud(620, 100, 0.8)
  ...
  /* 湖景 */
  lake(mom) {
    return scSky() + scSun(640, 80) + scCloud(180, 70) + scCloud(440, 50, 0.7)
      + scHills()
      ...
      + scTree(90, 315, 1) + scTree(730, 315, 0.85)
      + scDadKid(400, 318, 1, mom);
  },

  /* 红叶 */
  redleaf(mom) {
    return scSky() + scSun(660, 80) + scCloud(200, 70, 0.9)
      + scHills()
      + `<path d="M80 420 Q 260 330 420 350 T 740 300" stroke="#E9D8A6" stroke-width="24" fill="none" stroke-linecap="round"/>`
      + scTree(110, 335, 1.2, SC.leafRed) + scTree(300, 325, 0.9, SC.leafOrg)
      + scTree(560, 315, 1.1, SC.leafRed) + scTree(720, 300, 0.9, SC.leafOrg)
      + Array.from({ length: 8 }, (_, i) => {
          const lx = 120 + i * 90, ly = 150 + (i * 47) % 130;
  ...
  /* 首页头图（全家福场景） */
  hero(mom) {
    return scSky() + scSun(110, 90) + scCloud(330, 60) + scCloud(600, 100, 0.8)
      + scHills() + scRiver(340, 80)
      + scPine(80, 330, 1.2) + scTree(720, 320, 1.1)
      + scTent(560, 360, 0.9) + scCampfire(700, 390, 0.6)
      + scCar(160, 386, 0.9)
      + scKite(470, 110, 0.9)
      + scFish(360, 380, 0.8)
...
function sceneSVG(theme, mom = false) {
...
const ACT_ICONS = {
  /* 出发：小汽车 */
  depart: () => aiBadge(SC.dadShirt, `<g transform="translate(24,26) scale(0.32)">
  ...
  /* 骑行 */
  cycle: () => aiBadge(SC.dadShirt, `<g transform="translate(24,28) scale(0.62)" stroke="${SC.dadShirt}" stroke-width="6" fill="none" stroke-linecap="round">
