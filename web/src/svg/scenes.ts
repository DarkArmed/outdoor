...
export function scTree(x: number, y: number, s = 1, color: string = SC.leaf): string {
  return `
...
export function scPine(x: number, y: number, s = 1, color: string = SC.grassDk): string {
  return `
  <g transform="translate(${x},${y}) scale(${s})">
...
export function scPerson(x: number, y: number, s: number, shirt: string, opts: { hat?: string; hair?: string } = {}): string {
